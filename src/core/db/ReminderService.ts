...
import type { CompletionEntry } from '../store/statsStore';
...
export class ReminderService {
  ...
  static async updateReminder(id: string, changes: Partial<Reminder>): Promise<void> {
    ...
    if (changes.soundMode !== undefined) {
      fields.push('sound_mode = ?');
      values.push(changes.soundMode);
    }
    if (changes.snoozeMins !== undefined) {
      fields.push('snooze_mins = ?');
      values.push(changes.snoozeMins);
    }
    if (changes.character !== undefined) {
      fields.push('character = ?');
      values.push(changes.character);
    }
