...
export class ReminderService {
  static async getAllReminders(): Promise<Reminder[]> {
    ...
    if (res.values) {
      for (const row of res.values) {
        // Fetch schedules for each reminder
  ...
  static async seedIfEmpty(): Promise<void> {
    ...
    if (count === 0) {
      console.log('[ReminderService] No reminders found. Seeding default data...');
  ...
  static async addReminder(r: Reminder): Promise<void> {
    const db = dbManager.connection;
    await db.run(
      `INSERT INTO reminders (id, category, label, enabled, archived, sound_mode, snooze_mins, character, custom_message, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        r.id, 
        r.category, 
  ...
  static async updateReminder(id: string, changes: Partial<Reminder>): Promise<void> {
    ...
    // Simplistic update for now - only top level fields
    const fields: string[] = [];
    const values: any[] = [];
    ...
    // Handle nested schedules update (replace strategy)
      ...
      for (const s of changes.schedules) {
        await db.run(
          ...
          [
            ...
            s.startTime, 
            s.endTime,
            s.notifId ?? Math.floor(Math.random() * 2000000)
          ]
        );
      }
    }
  ...
  static async deleteReminder(id: string): Promise<void> {
    const db = dbManager.connection;
    // Note: schedules are deleted automatically due to ON DELETE CASCADE
